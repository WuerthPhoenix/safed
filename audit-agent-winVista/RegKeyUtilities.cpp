...
BOOL MyWriteProfileWString(LPCTSTR lpszSection, WCHAR * lpszEntry, LPCWSTR lpszString)
{
	HKEY hSecKey = MyGetSectionKey(lpszSection);
	DWORD size = (wcslen(lpszString)+1)*2;
	if (hSecKey == NULL)
		return TRUE;
	LONG lResult = RegSetValueExW(hSecKey, (LPCWSTR)lpszEntry, NULL, REG_SZ,
		(unsigned char *)lpszString, size);
...
BOOL MyGetProfileWString(LPCTSTR lpszSection, WCHAR * lpszEntry, LPCWSTR lpszString, DWORD dwStringBuffer)
{
	HKEY hSecKey = MyGetSectionKey(lpszSection);
	if (hSecKey == NULL)
		return false;
	DWORD dwSize = dwStringBuffer;
	DWORD dwType=0;
	LONG lResult = RegQueryValueExW(hSecKey, (LPCWSTR)lpszEntry, NULL, &dwType,
		(LPBYTE)lpszString, &dwSize);
	RegCloseKey(hSecKey);
	if (dwType != REG_SZ) {
		wcsncpy_s((WCHAR *)lpszString,dwStringBuffer,L"",_TRUNCATE);
		return false;
	}
	return lResult == ERROR_SUCCESS;
}
