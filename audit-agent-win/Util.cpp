...
void PrintIADSObject(IADs * pIADs)
{
	...
	LogExtMsg(DEBUG_LOG," NAME: %s\n", bsResult);
	SysFreeString(bsResult);

	pIADs->get_ADsPath(&bsResult); 
	LogExtMsg(DEBUG_LOG," ADSPATH: %s\n", bsResult);
	SysFreeString(bsResult);

	/*
	pIADs->get_Class(&bsResult); 
	LogExtMsg(DEBUG_LOG," CLASS: %s\n", bsResult);
	SysFreeString(bsResult);

	pIADs->get_Parent(&bsResult); 
	LogExtMsg(DEBUG_LOG," PARENT: %s\n", bsResult);
	SysFreeString(bsResult);

	pIADs->get_GUID(&bsResult); 
	LogExtMsg(DEBUG_LOG," GUID: %s\n", bsResult);
	SysFreeString(bsResult);

	pIADs->get_Schema(&bsResult); 
	LogExtMsg(DEBUG_LOG," SCHEMA: %s\n", bsResult);
	SysFreeString(bsResult);	
	*/
}
...
HRESULT CheckErrorAD(LPWSTR txt, HRESULT hr){
	...
	wprintf(L"\n");
	LogExtMsg(ERROR_LOG,"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n", hr);
	LogExtMsg(ERROR_LOG,"An error occurred in %s, HRESULT: %x\n",txt, hr);
	...
	if (HRESULT_FACILITY(hr)==FACILITY_WIN32)
	{
		...
		{
			LogExtMsg(ERROR_LOG,"Error Code: %d Error Text: %ws Provider: %ws\n", dwLastError, szErrorBuf, szNameBuf);
		}
	}
	LogExtMsg(ERROR_LOG,"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n\n", hr);
