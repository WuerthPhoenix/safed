...
int Status_Page(char *source, char *dest, int size)
{
	...
	pos=strstr(getSADStatus(),"failed!");
	char color2[6]="green";
	if(pos){
		strcpy(color2,"red");
	}	
	...
	snprintf_s(dest,size,_TRUNCATE,"%s<center><b><font color=%s size=-1>%s</font></b></center><p>" \
								   ...
								   ,dest,color,socketStatus,color2,getSADStatus(), initStatus, e_initStatus, lastConnectionStatus,setConfigStatus,getConfigStatus);
...
int Network_Set(char *source, char *dest, int size)
{
	...
	network_struct.dw_SocketType = 0;
...
int GetSysAdmin(SOCKET http_socket, gnutls_session session_https){
	...
	if(getSAStr())
		if(WEBSERVER_TLS) 
			sendTLS(getSAStr(),session_https);
		else
			send(http_socket,getSAStr(),(int)strlen(getSAStr()),0);
	else 
		if(WEBSERVER_TLS) 
			sendTLS(msg,session_https);
		else
